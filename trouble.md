...
1. 通过观察指示灯判断键盘工作情况：
    -  如果插入USB线后，亮USB工作指示灯，说明键盘基本没有问题
    -  如果指示灯无任何指示，考虑是否固件刷写失败或键盘已经损坏
2. 可按照以下步骤检查键盘硬件是否损坏：
...
3. 如果遇到了蓝牙方面的问题，可以尝试 :关闭键盘后重新启动键盘、重新绑定蓝牙或重启电脑。
4. 如果还无法解决问题，可以尝试: [重置键盘](faq.md#如何重置键盘)。
5. 支持线刷的新键盘可以勾选烧录工具中"烧录时擦除蓝牙固件"选项，并更新`完整蓝牙固件`进行修复。
6. 如还无法解决故障，可参考下面的具体情形进行处理，或请[联系我们](introduce.md#联系我们)。
...
3. 如果是升级新固件后导致的
    ...
    - 如果是老硬件（2020年6月以前）升级后USB无法输出，可能是未同步升级USB固件，请[联系我们](introduce.md#联系我们)获取USB固件。
...
1. 可能是下载的固件不对应，比如下载了其他键盘的固件，或者固件的版本不对应。
    - 如键盘是Omega50，却升级了Omega45的键盘固件
    - 如键盘是Farad rev.B版，却升级了Farad rev.A的键盘固件
    - 请更新正确的固件即可解决问题。

2. 确定固件是正确的对应的固件
    - 蓝牙固件有机率刷写失败，可以尝试重新刷写固件。如果指示灯能正常运作，方可确认蓝牙固件刷写正确。
    - 升级USB固件后，将电脑上的USB设备删除，并重新插入USB线进行识别及驱动安装。
    - 升级蓝牙固件后，将电脑上的蓝牙设备删除，并将键盘上的绑定清空，然后重新进行绑定。
...
3. 如果上述方式无法确认问题的话，也可能是因为线路短接、静电击毁等原因导致蓝牙故障，请[联系我们](introduce.md#联系我们)。
