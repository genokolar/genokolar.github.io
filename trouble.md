...
4. 支持线刷的新键盘可以尝试 [CMSIS-DAP烧录](upgrade2.md#使用板载调试器强制更新蓝牙完整固件)更新整个蓝牙固件进行修复。
...
3. 若还是无法正常开机，支持线刷的新键盘可以尝试 [CMSIS-DAP烧录](upgrade2.md#使用板载调试器强制更新蓝牙完整固件)更新整个蓝牙固件进行修复。
...
4. 上述无法解决的话，支持线刷的新键盘可以尝试 [CMSIS-DAP烧录](upgrade2.md#使用板载调试器强制更新蓝牙完整固件)更新整个蓝牙固件进行修复。
...
1. 确保蓝牙固件是新版固件，否则请升级蓝牙固件：访问 [相关下载](down/download.md) 下载新版固件，然后 [更新蓝牙固件](upgrade.md#更新蓝牙固件)

2. 确保USB固件是新版固件：如果升级蓝牙固件后可以识别，不需要升级USB固件，如果还是无法识别，请[升级USB固件](upgrade.md#更新USB固件)，并删除电脑上USB键盘，重新插入键盘识别

3. 必须开启客户端：[lkb-configurator-server](down/download.md#配置工具)
