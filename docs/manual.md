...
### 蓝牙连接设备
如需通过 蓝牙连接 进行输入，你需要完成 [配对蓝牙设备](#配对蓝牙设备) 的过程。

### 2.4G无线 连接设备
如需通过 2.4G无线 进行输入，请将 2.4G接收器 插入设备的USB口，并将键盘切换到2.4G无线模式即可。（暂行方案）
...
| 切换蓝牙设备     | <kbd>Lshift</kbd>+<kbd>Rshift</kbd>+<kbd>Q</kbd>/<kbd>W</kbd>/<kbd>E</kbd>        |可以在已配对的蓝牙设备之间进行切换，<kbd>Q</kbd>/<kbd>W</kbd>/<kbd>E</kbd>代表不同蓝牙连接通道 |
| 重启蓝牙广播     | <kbd>Lshift</kbd>+<kbd>Rshift</kbd>+<kbd>R</kbd>        | 重新开启蓝牙广播，用于手动进行连接或切换设备后配对。|
| 进入DFU     | <kbd>Lshift</kbd>+<kbd>Rshift</kbd>+<kbd>B</kbd>        |重启到DFU刷机模式。<br>也可长按PCB背部多功能按钮4秒以上后松手重启到DFU刷机模式。 |
| 清空当前配对  |<kbd>Lshift</kbd>+<kbd>Rshift</kbd>+<kbd>O</kbd>    |清空当前蓝牙设备配对信息。仅清空当前设备，其余配对设备不会清空。|
...
- 4、在需要配对键盘的设备上搜索蓝牙键盘，搜索到相应蓝牙键盘名称后，将新的设备配对到键盘蓝牙通道2️⃣（<kbd>W</kbd>键 ）；
...
GT系列蓝牙键盘单色RGB轴灯版默认采用轴灯作为指示，不同颜色代表不同状态：
...
GT系列蓝牙键盘炫彩RGB轴灯版默认采用某颗轴灯作为指示，不同颜色代表不同状态：
