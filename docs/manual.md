...
如果通过软关机后，可以通过插入USB线 / 按击一下背部多功能按钮 / 按键旋钮中键 实现开机。
...
!!! tip "电量百分比仅供参考"
    ...
    由于测量方式存在误差，电量百分比可能无法达到 100% 或者 数值有所波动，这是正常现象。
...
短接PCB背部K1位置两个焊盘，并插入USB线可进入USP ISP模式; 
...
### RGB轴灯版PCB
RGB轴灯版PCB采用WS2812 RGB灯，支持各种丰富绚丽的灯效（同QMK灯效），暂时不支持指示灯功能。
