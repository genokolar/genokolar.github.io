...
!!! tip "硬件损坏的可能性极小，99%的问题都可以通过[刷写完整固件修复](upgrade.md#cmsis-dap)，如无法确定什么问题，可以无脑先下载CMSIS-DAP刷机包先[刷写完整固件](upgrade.md#cmsis-dap)尝试是否可以修复问题"
...
3. 如果第一步中指示灯无指示，但是第二步中判断出USB芯片没有问题：
    ...
    -  如果指示灯还是无任何指示，可以尝试刷写`完整键盘主控固件`修复键盘 [如何修复键盘](upgrade.md#cmsis-dap)
...
2. 如果无线（包括蓝牙与2.4G）无法正常输出，首先请确认正确进行了[配对蓝牙设备](manual.md#配对蓝牙设备) 或 [2.4G接收器配对](manual.md#2.4G接收器配对)，如配对后也无法正常输出，请重新拔插2.4G接收器或参考：[如何改善无线连接稳定性](faq.md#如何改善无线连接稳定性)
   
3. 如果是升级新固件后导致的,请查阅[升级固件后键盘无法正常工作](#升级固件后键盘无法正常工作)
     
4. 如果上述步骤还无法解决问题，可以尝试: [重置键盘](faq.md#如何重置键盘) 或 尝试更新`键盘主控完整固件`进行修复

5. 部分已知异常问题，如：睡眠耗电异常、刷机容易失败、唤醒按键功能无法启用等，可尝试拔除电池给键盘断电，然后插入电池的方式重启解决。

<span id="升级固件后键盘无法正常工作">升级固件后键盘无法正常工作</span>
-------------------
如刷写固件后睡眠耗电异常、刷机容易失败、唤醒按键功能无法启用、RGB无法正常运行关闭等，可尝试拔除电池给键盘断电，然后插入电池的方式重启解决。

如刷写固件后指示灯未正常运作、无法输出按键 或 输出按键与实际按键不符，可考虑主控固件刷写出错
...
3. 如固件刷写正确、无误
    ...
    - 尝试拔除电池给键盘断电，然后插入电池的方式重启解决一些异常问题。
    - 如还无法解决问题，可考虑勾选烧录工具中"擦除蓝牙芯片"选项，并更新`键盘主控完整固件`进行修复

4. 如错误将2.4G接收器固件刷入键盘，请重新下载正确的键盘固件，并按照[2.4G接收器固件刷写操作](keyboard/receiver.md#_2)重新刷入正确的键盘固件。
...
2. 如果没有升级固件，可能是选择了不正确的默认层。可以尝试`恢复默认层`的设置：先 :`睡眠键盘`，再`唤醒键盘`，即可恢复默认层。
...
1. 确保固件支持。
   - 2020年4月后固件支持使用配置工具
   - 2024年11月11日后固件支持USB和蓝牙免服务端，通过Webhid使用配置工具
   - 2025年1月11日后固件支持2.4G免服务端使用配置工具
   - 升级键盘主控固件：访问 [相关下载](down/download.md) 下载新版固件，然后 [更新键盘主控固件](upgrade.md#更新键盘主控固件)
   - 升级USB固件：访问 [相关下载](down/download.md) 下载新版固件，然后[更新USB固件](upgrade.md#更新USB固件)，如有必要，请删除电脑上USB键盘，重新插入键盘识别

2. 如果采用服务端方式接入，必须开启服务端：lkb-configurator-server [前往下载](down/download.md)，并且需要每年初更新一次配置工具

3. 不要使用USB集线器，直接将键盘线插入电脑USB口；关闭代理，如有必要关闭防火墙

4. 检查识别情况：开启服务端工具的同时，在浏览器中访问http://localhost:5000/api/device
    - (1).如果显示：无法访问此页面，说明配置工具没有正常启用，考虑重启或更新配置工具
    - (2).如果显示：[]，说明配置工具正常，但是键盘未被识别。
    - (3).如果显示：[{"id":"4992B144","vid":17254,....]，说明键盘正确识别

5. 显示“配置工具没有响应” 或 无法访问网站
    - 请首先确保电脑正常联网，并关闭代理、防火墙等，使配置工具可正常访问网络
    - 刷新网页重新尝试

6. 如上述方法无法解决问题，也可采用Chrome内核的浏览器直接访问[🌍配置工具网站](https://keyboard.lotlab.org/)，尝试用WebHID方式直接连接配置工具（需要2024年11月11日后固件）
