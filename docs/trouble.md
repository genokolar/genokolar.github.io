...
3. 如果第一步中指示灯无指示，但是第二步中判断出USB芯片没有问题：
    ...
    -  如果刷写`完整蓝牙固件`时找不到键盘，排除连接问题后，可能是蓝牙主控或USB芯片已经损坏（硬件损坏的可能性极小）。
4. 如第二步中判断出USB芯片存在问题：
...
2. 如果蓝牙无法正常输出，首先请确认正确进行了[配对蓝牙设备](manual.md#配对蓝牙设备)，如配对后也无法正常输出，请参考：[如何改善蓝牙连接稳定性](faq.md#如何改善蓝牙连接稳定性)
...
3. 如固件刷写正确、无误
    ...
    - 升级蓝牙固件后，将电脑上的蓝牙设备删除，并将键盘上的配对清空，然后重新进行配对。
...
4. 安装版配置工具显示空白页面 或 其他访问异常
    ...
    - 每年至少需要更新一次配置工具，如长时间未更新，请检查是否需要更新配置工具
    - 如无法确认情况，但属于第4条第(3)种情况，可在开启配置工具的同时，浏览器访问keyboard.lotlab.org
