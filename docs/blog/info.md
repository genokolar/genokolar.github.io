...
2. 接收器配对采用开放模式，不需要特别设定，不需要程序干预
3. 与蓝牙、USB可以实现无缝切换
4. 所有PCB升级固件后，都可以购买2.4G接收器后实现2.4G功能

### 2.4G如何配对

1. 将接收器插入电脑USB口通电
2. 键盘切换到2.4G无线模式
3. 键盘未配对：按下双shift+R启动绑定通讯，即可配对完成
4. 键盘已配对：按下双shift+O删除绑定信息，按下双shift+R启动绑定通讯，即可配对到新的接收器
...
1. 将接收器插入电脑USB口
2. 浏览器访问[接收器控制面板](https://wiki.glab.online/2.4G/index.html)
